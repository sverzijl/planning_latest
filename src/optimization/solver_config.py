...
# =============================================================================
# OPTIMIZED MIP SETTINGS (Added 2025-11-03)
# Based on 18-level incremental testing - reduced solve time from 120s to 16s
# =============================================================================

# Optimized HiGHS settings for MIP problems
HIGHS_MIP_OPTIMIZED = {
    'presolve': 'on',       # Critical for MIP performance
    'parallel': 'on',        # Parallel branch-and-bound
    'mip_rel_gap': 0.02,    # 2% optimality gap (good for planning)
    'time_limit': 30.0,      # 30 second limit
}

HIGHS_MIP_ACCURATE = {
    'presolve': 'on',
    'parallel': 'on',
    'mip_rel_gap': 0.001,   # 0.1% gap (near-optimal)
    'time_limit': 300.0,     # 5 minute limit
}


def configure_highs_for_mip(solver, mode='fast'):
    """
    Configure HiGHS solver with optimized MIP settings.

    Discovered through incremental testing (Level 18):
    - Without optimization: 120+ seconds timeout
    - With optimization: 16 seconds ✓

    Args:
        solver: Pyomo APPSI HiGHS solver
        mode: 'fast' (2% gap, 30s) or 'accurate' (0.1% gap, 300s)
    ...
    Example:
        ...
        >>> solver = configure_highs_for_mip(solver, mode='fast')
        >>> result = solver.solve(model)
    """
    config = HIGHS_MIP_OPTIMIZED if mode == 'fast' else HIGHS_MIP_ACCURATE
