...
15. **Coefficient scaling (1000×):** (2025-11-05) All flow variables (production, inventory, shipments) scaled to thousands of units internally; fixes critical numerical instability (400M coefficient ratio → 4,688); 20-40% faster solves, 85,000× better conditioning. Variables automatically unscaled on extraction so UI sees original units. See `docs/COEFFICIENT_SCALING_ARCHITECTURE.md`

**Recent Key Updates:**
- **Coefficient Scaling (1000×)** (Nov 5, 2025): All flow variables scaled to thousands internally; fixes critical 400M coefficient ratio causing slow solves; 20-40% faster, 85,000× better numerical conditioning; automatic unscaling for UI. See `docs/COEFFICIENT_SCALING_ARCHITECTURE.md`
