#!/usr/bin/env python3
"""Check disposal and inventory by day."""
...
print("="*80)
print("DISPOSAL AND INVENTORY BY DAY")
print("="*80)
...
if result.is_optimal():
    ...
    # Check by day
    dates = sorted(set(t for (_, _, _, t) in pyomo_model.inventory))

    print(f"\nDay-by-day analysis:")
    for day in dates:
        day_disposal = 0
        if hasattr(pyomo_model, 'disposal'):
            for (node, prod, state, t) in pyomo_model.disposal:
                if t == day:
                    val = value(pyomo_model.disposal[node, prod, state, t])
                    if val: day_disposal += val

        day_inventory = 0
        for (node, prod, state, t) in pyomo_model.inventory:
            if t == day:
                val = value(pyomo_model.inventory[node, prod, state, t])
                if val: day_inventory += val

        print(f"  {day}: inventory={day_inventory:,.0f}, disposal={day_disposal:,.0f}")

    print(f"\nInitial inventory should be {sum(initial_inv.values()):,.0f}")
