"""
Test shortage fix with allow_shortages=True to see if shortages are reported.
"""
import sys
...
print("=" * 80)
print("SHORTAGE FIX VERIFICATION (ALLOW_SHORTAGES=TRUE)")
print("=" * 80)
...
if production_diff < 100_000 and cost_diff < 1_000_000:
    ...
    shortage_pct = total_shortages / total_demand * 100
    print(f"\n⚠️  Model reports shortages")
    print(f"   Shortages: {total_shortages:,.0f} units ({shortage_pct:.1f}% of demand)")
    print(f"   This suggests the problem may be capacity-constrained")
