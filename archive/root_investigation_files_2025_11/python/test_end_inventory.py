#!/usr/bin/env python3
"""Check end-of-horizon inventory."""

from datetime import date, timedelta
from src.parsers.multi_file_parser import MultiFileParser
from src.parsers.inventory_parser import InventoryParser
from src.optimization.legacy_to_unified_converter import LegacyToUnifiedConverter
from src.optimization.sliding_window_model import SlidingWindowModel
from tests.conftest import create_test_products
from pyomo.environ import value

# Parse data
parser = MultiFileParser(
    forecast_file='data/examples/Gluten Free Forecast - Latest.xlsm',
    network_file='data/examples/Network_Config.xlsx'
)
forecast, locations, routes, labor_calendar, truck_schedules, cost_params = parser.parse_all()

# Convert
mfg_site = next((loc for loc in locations if loc.id == '6122'), None)
converter = LegacyToUnifiedConverter()
nodes, unified_routes, unified_trucks = converter.convert_all(
    manufacturing_site=mfg_site, locations=locations, routes=routes,
    truck_schedules=truck_schedules, forecast=forecast
)

# Parse inventory
alias_resolver = parser.parse_product_aliases()
inv_parser = InventoryParser('data/examples/inventory_latest.XLSX', product_alias_resolver=alias_resolver)
inventory_data = inv_parser.parse()
inventory_data.snapshot_date = date(2025, 10, 16)

# Convert
inv_2tuple = inventory_data.to_optimization_dict()
initial_inv = {}
loc_dict = {loc.id: loc for loc in locations}
for (location, product), quantity in inv_2tuple.items():
    loc_node = loc_dict.get(location)
    if loc_node and str(loc_node.storage_mode) == 'frozen':
        state = 'frozen'
    else:
        state = 'ambient'
    initial_inv[(location, product, state)] = quantity

# 1-week
start = date(2025, 10, 17)
end = start + timedelta(days=6)

product_ids = sorted(set(entry.product_id for entry in forecast.entries))
products = create_test_products(product_ids)

print("="*80)
print("END-OF-HORIZON INVENTORY CHECK")
print("="*80)

model = SlidingWindowModel(
    nodes=nodes, routes=unified_routes, forecast=forecast,
    products=products, labor_calendar=labor_calendar,
    cost_structure=cost_params, start_date=start, end_date=end,
    truck_schedules=unified_trucks, initial_inventory=initial_inv,
    inventory_snapshot_date=inventory_data.snapshot_date,
    allow_shortages=True, use_pallet_tracking=False, use_truck_pallet_tracking=True
)

result = model.solve(solver_name='appsi_highs', time_limit_seconds=60, mip_gap=0.01, tee=False)

print(f"\nSolve: {result.termination_condition}")

if result.is_optimal():
    pyomo_model = model.model

    # Check end inventory
    last_day = end
    total_end_inv = 0
    for (node_id, prod, state, t) in pyomo_model.inventory:
        if t == last_day:
            val = value(pyomo_model.inventory[node_id, prod, state, t])
            if val and abs(val) > 0.01:
                total_end_inv += val

    total_init_inv = sum(initial_inv.values())

    print(f"\nInitial inventory (day 0): {total_init_inv:,.0f} units")
    print(f"End inventory (day 7): {total_end_inv:,.0f} units")
    print(f"Difference: {total_end_inv - total_init_inv:,.0f} units")

    # Calculate expected waste cost
    waste_mult = cost_params.waste_cost_multiplier or 0
    prod_cost = cost_params.production_cost_per_unit or 1.30
    expected_waste_cost = waste_mult * prod_cost * total_end_inv

    print(f"\nWaste cost calculation:")
    print(f"  Multiplier: {waste_mult}")
    print(f"  Production cost: ${prod_cost}/unit")
    print(f"  End inventory: {total_end_inv:,.0f} units")
    print(f"  Expected waste cost: ${expected_waste_cost:,.2f}")

print("="*80)
