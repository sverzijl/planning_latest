...
for leg, prod, delivery_date in pyomo_model.shipment_leg:
    if delivery_date == first_date:
        qty = value(pyomo_model.shipment_leg[leg, prod, delivery_date])
        if qty > 0.01:
            origin, dest = leg
...
if arrivals_first_date:
    total_invalid = sum(x[3] for x in arrivals_first_date)
    print(f"""
Found {len(arrivals_first_date)} shipments arriving on first date
Total quantity: {total_invalid:,.1f} units
...
This should be added when building the model variable constraints.
""")
else:
    print("\nNo arrivals on first date - checking if the bug exists in later dates...")
