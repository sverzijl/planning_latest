#!/usr/bin/env python3
"""Test: Does model solve with ONLY Lineage inventory?"""
...
# KEEP ONLY Lineage inventory (storage_location=4070)
...
# Keep ONLY Lineage
inventory_snapshot.entries = lineage_entries
print(f"After keeping only Lineage: {len(inventory_snapshot.entries)} entries")
...
print("\n" + "="*80)
print("TEST: SlidingWindowModel with ONLY Lineage frozen inventory")
print("="*80)
...
if result.success:
    print("✓ OPTIMAL - Lineage frozen inventory works alone!")
else:
    print("✗ INFEASIBLE - Lineage frozen inventory causes the problem")
