#!/usr/bin/env python3
"""Test: Real inventory but ONLY at plant (6122), remove all others."""
...
# Keep ONLY plant (6122) inventory with storage_location=4000
...
inventory_snapshot.entries = [
    ...
    if e.location_id == '6122' and e.storage_location == '4000'
]

print(f"Original: {orig_count} entries, {orig_total:.0f} units")
print(f"Plant only: {len(inventory_snapshot.entries)} entries, {sum(e.quantity for e in inventory_snapshot.entries):.0f} units")
...
print("\n" + "="*80)
print("TEST: Real inventory at plant only (no hubs, no Lineage)")
print("="*80)
...
if result.success:
    print("✓ OPTIMAL - Plant inventory works!")
    print("→ Issue is with hub or Lineage inventory")
else:
    print("✗ INFEASIBLE - Even plant-only inventory fails")
